...
final class AuthenticationManager {
    ...
    func getAuthenticatedUser() throws -> AuthResultDataModel {
...
extension AuthenticationManager {
    @discardableResult
    func signInAnonymous() async throws -> AuthResultDataModel {
        let authDataResult = try await Auth.auth().signInAnonymously()
        return AuthResultDataModel(user: authDataResult.user)
    }
    
    func linkEmail(email: String, password: String) async throws -> AuthResultDataModel {
        let credential = EmailAuthProvider.credential(withEmail: email, password: password)
        
        return try await linkCredential(credential: credential)
    }

    func linkGoogle(tokens: GoogleSignInResultModel) async throws -> AuthResultDataModel {
        let credential = GoogleAuthProvider.credential(
            withIDToken: tokens.idToken,
            accessToken: tokens.accessToken
        )
    ...
    func linkApple(tokens: SignInWithAppleResult) async throws -> AuthResultDataModel {
        let credential = OAuthProvider.credential(
            withProviderID: AuthProviderOption.apple.rawValue,
            idToken: tokens.token,
            rawNonce: tokens.nonce
    ...
    private func linkCredential(credential: AuthCredential) async throws -> AuthResultDataModel {
        guard let user = Auth.auth().currentUser else {
            throw URLError(.badURL)
        }
